...
(async () => {
  ...
  const searchForUsername = async (usernameToSearch) => {
    try {
      ...
      // Wait for the search results to appear (class "result_*")
      await page.waitForSelector('[class^="result_"]', { timeout: 30000 });

      // Extract all results
      const results = await page.$$('[class^="result_"]');
      ...
      if (results.length > 1) {
        console.log(
          `Multiple results found for the search query "${usernameToSearch}".`
        );
        results.map(async (result) => {
          const _result = await result.$('span[class^="username_"]');
          console.log(await _result.evaluate((element) => element.textContent));
        });
        // return;
      }

      const result = results.find(async (result) => {
        const _result = await result.$('span[class^="username_"]');
        return (
          (await _result.evaluate((element) => element.textContent)) ===
          usernameToSearch
        );
      });
      ...
      // Extract avatar, username, and nickname (if available)
      const avatarElement = await result.$('img[class^="avatar_"]');
      const usernameElement = await result.$('span[class^="username_"]');
      const nicknameElement = await result.$('span[class^="match_"]');
